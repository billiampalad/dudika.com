...
<div id="detailModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white w-full max-w-2xl rounded-lg shadow-xl transform transition-all duration-300 scale-95 opacity-0 max-h-[90vh] flex flex-col" id="detailModalContent">
        <div class="flex justify-between items-center px-6 py-4 border-b bg-gray-50">
            ...
            <button onclick="closeDetailModal()" class="text-gray-400 hover:text-gray-500">&times;</button>
        </div>
        <form id="evaluasiForm" class="flex-grow overflow-y-auto p-6 space-y-6">
            ...
            <div>
                ...
                <textarea id="txtRekomUtkPerbaikan" name="txtRekomUtkPerbaikan" rows="4" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"></textarea>
            </div>

            <div class="flex justify-end space-x-3 pt-5 border-t">
                <button type="button" onclick="closeDetailModal()" class="px-4 py-2 border rounded-md text-sm">Batal</button>
                <button type="submit" class="px-4 py-2 border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">Simpan Evaluasi</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const apiCall = async (action, options = {}) => {
            const url = `evaluasi_action.php?action=${action}`;
            try {
        ...
        const createRatingControl = (key, label, value) => {
            let starsHTML = '';
            for (let i = 1; i <= 5; i++) {
                starsHTML += `<i class="${i <= value ? 'fas fa-star text-yellow-400' : 'far fa-star text-gray-300'}"></i>`;
            }
            return `
            <div class="rating-group">
                <label class="block text-sm font-medium text-gray-700">${label}</label>
                <div class="flex items-center justify-between gap-4 mt-1">
                    <div class="star-display flex space-x-1 text-xl">${starsHTML}</div>
                    <input type="range" name="${key}" min="1" max="5" value="${value}" class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer">
                </div>
            </div>`;
        };
        ...
        window.showDetailModal = async (triggerElement, id) => {
            ...
            modalTitle.textContent = `Evaluasi Kinerja: ${nama_kegiatan}`;
        ...
        const loadRadarChart = async () => {
            ...
            const datasets = result.data.programs.map((prog, index) => ({
                label: prog.name,
                data: prog.scores,
                fill: true,
        ...
        ratingContainer.addEventListener('input', e => {
            if (e.target.matches('input[type="range"]')) {
                const rating = parseInt(e.target.value, 10);
                const starContainer = e.target.closest('.rating-group').querySelector('.star-display');
                let starsHTML = '';
                for (let i = 1; i <= 5; i++) {
                    starsHTML += `<i class="${i <= rating ? 'fas fa-star text-yellow-400' : 'far fa-star text-gray-300'}"></i>`;
                }
