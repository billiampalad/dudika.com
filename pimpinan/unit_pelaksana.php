...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-gray-100">
        <form class="relative max-w-md flex items-center" onsubmit="searchUnitPelaksana(event)">
...
<script>
    ...
    function showEditModal(triggerElement) {
        ...
        setTimeout(() => {
            ...
            document.getElementById('edit_jurusan').focus();
        }, 10);
    }
    ...
    function deleteData(id, isInUse) {
        if (isInUse) {
            Swal.fire({
                ...
                html: `Unit Pelaksana ID <b>${id}</b> sedang digunakan pada data kegiatan dan tidak bisa dihapus.`
            });
            return;
        }
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                try {
                    const response = await fetch('pimpinan/unit_pelaksana_action.php', {
                        method: 'POST',
                        body: formData
                    });
    ...
    function exportData() {
        window.open('export_excel.php?action=export_unit_pelaksana', '_blank');
    }
        ...
        try {
            ...
            if (res.status === 'success') {
                ...
                await Swal.fire({
                    ...swalWithTailwind,
                    title: 'Berhasil!',
                    text: `Data berhasil ${isEditing ? 'diperbarui' : 'ditambahkan'}.`,
                    icon: 'success',
                    timer: 1500,
                    showConfirmButton: false
                });
    ...
    async function searchUnitPelaksana(event) {
        ...
        try {
            ...
            const response = await fetch(`pimpinan/unit_pelaksana_action.php?action=search&keyword=${encodeURIComponent(keyword)}`);
            ...
            if (data.status === 'success') {
                ...
                updateUnitPelaksanaTable(data.data);
            } else {
                Swal.fire({
                    ...swalWithTailwind,
                    icon: 'error',
    ...
    function updateUnitPelaksanaTable(data) {
        ...
        if (data.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="4" class="text-center py-4 text-gray-500">Tidak ada data ditemukan.</td></tr>';
        ...
        data.forEach(item => {
            const jurusan_name = item.txtNamaUnitPelaksPolimdo.replace('Jurusan ', '');
            const is_in_use = false; // Karena kita tidak melakukan pengecekan saat search

            const row = document.createElement('tr');
            row.className = 'hover:bg-gray-50 transition-colors';
            row.innerHTML = `
            <td class="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-500">${item.IdUnitPelaksana}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${item.txtNamaUnitPelaksPolimdo}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${item.txtNamaStafAdminUnit}</td>
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex justify-center space-x-2">
                    <button onclick="editData(this, '${item.IdUnitPelaksana}', '${item.txtNamaUnitPelaksPolimdo.replace(/'/g, "\\'")}', '${item.txtNamaStafAdminUnit.replace(/'/g, "\\'")}', '${jurusan_name}')" 
                        class="text-indigo-600 hover:text-indigo-900 transition-colors" title="Edit">
                        <i class="fas fa-edit fa-lg"></i>
                    </button>
                    <button onclick="deleteData('${item.IdUnitPelaksana}', ${is_in_use})" 
                        class="text-red-600 hover:text-red-900 transition-colors" title="Hapus">
                        <i class="fas fa-trash fa-lg"></i>
                    </button>
                </div>
        ...
        data.forEach(item => {
            ...
            card.innerHTML = `
            <div class="flex justify-between items-start">
                <h3 class="text-sm font-semibold text-gray-800">${item.txtNamaUnitPelaksPolimdo}</h3>
                <div class="flex space-x-2">
                    <button onclick="editData(this, '${item.IdUnitPelaksana}', '${item.txtNamaUnitPelaksPolimdo.replace(/'/g, "\\'")}', '${item.txtNamaStafAdminUnit.replace(/'/g, "\\'")}', '${jurusan_name}')" 
                        class="text-indigo-600 hover:text-indigo-500 transition-colors" title="Edit">
                        <i class="fas fa-edit"></i>
            ...
            <div class="mt-3 space-y-2 text-sm">
                <div class="flex items-center">
                    <i class="fas fa-tag text-gray-400 mr-2 w-4"></i>
                    <span class="text-gray-500 font-mono">${item.IdUnitPelaksana}</span>
                </div>
                <div class="flex items-center">
                    <i class="fas fa-user-tie text-gray-400 mr-2 w-4"></i>
                    <span class="text-gray-600">${item.txtNamaStafAdminUnit}</span>
                </div>
            </div>
        `;
    ...
    addDataForm.addEventListener('submit', (e) => {
        e.preventDefault();
        handleFormSubmit(addDataForm, 'pimpinan/unit_pelaksana_action.php');
    });
    editDataForm.addEventListener('submit', (e) => {
        e.preventDefault();
        handleFormSubmit(editDataForm, 'pimpinan/unit_pelaksana_action.php');
    });
