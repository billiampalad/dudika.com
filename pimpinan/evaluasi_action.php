...
function get_single_evaluation($koneksi) {
    ...
    if ($eval_result->num_rows === 0) {
        echo json_encode(['status' => 'error', 'message' => 'Data evaluasi tidak ditemukan.']); exit;
    }
...
function update_evaluation($koneksi) {
    ...
    if (empty($id_eval)) {
        echo json_encode(['status' => 'error', 'message' => 'ID Evaluasi tidak valid.']); exit;
    }
    ...
    try {
        ...
        $stmt1->bind_param('iiiiiis', 
            $_POST['txtSesuaiRencana'], $_POST['txtKualitasPelaks'], $_POST['txtKeterlibatanMtra'], 
            $_POST['txtEfisiensiPenggSbDya'], $_POST['txtKepuasanPhkTerkait'], $id_eval
        );
        $stmt1->execute();
        $stmt1->close();
...
function get_radar_chart_data($koneksi) {
    ...
    while ($row = $result->fetch_assoc()) {
        $programs[] = [
            'name' => $row['txtNamaKegiatanKS'],
            'scores' => [
                (int)$row['txtSesuaiRencana'],
                (int)$row['txtKualitasPelaks'],
                (int)$row['txtKeterlibatanMtra'],
                (int)$row['txtEfisiensiPenggSbDya'],
                (int)$row['txtKepuasanPhkTerkait']
            ]
        ];
    }
    ...
    echo json_encode(['status' => 'success', 'data' => ['programs' => $programs, 'colors' => $colors]]);
}
?>